...
{
	int s, e, i;
	for (s = 1; s <= NUM_NODE; s++) {//출발 노드 번호
		for (e = 1; e <= NUM_NODE; e++) {//도착 노드 번호
			matrix[s][e] = NOT_CONNECTED;
		}
	}

	for (i = 0; i < NUM_EDGE; i++) {//간선 정보 인덱스
...
int Init_Graph_Adjacency_list(void)
{
	int i, idx, cnt = 0;
	LIST* head, * np;
	for (i = 1; i <= NUM_NODE; i++) {//테스트 케이스 여러개인 경우 필요
		if (list[i].next != NULL) {//할당 받은 것 있으면 할당해제
...
#if 01
//전역변수 heap 활용
//lastnode 변수 활용(lastnode == LAST_NODE)이면 full
int Heap_Push_Minheap(int d) 
{
