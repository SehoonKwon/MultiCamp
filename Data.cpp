...
//		if (exam[i].id == p->id)
//			return -2;
//		else if (exam[i].id < p->id) break; //낮은 사번이므로 더 이상 비교 불필요
//	}
//
//	for (i = n - 1; i >= 0; i--)
//	{
...
//	}
//}
//void DataDel(void) {//자료삭제
//	int cmd, id, n, r;
//	SCORE tmp;
//	printf("[1] 사번(ID)으로 삭제 / [2] 이름으로 삭제 : ");
//	scanf("%d", &cmd);
//	if (cmd == 1) {
//		printf("삭제할 사번 입력하시오 : ");
//		scanf("%d", &id);
...
//		printf("[4] 전체인쇄\n");
//		printf("[5] 자료인쇄\n");
//		printf("[6] 정렬(성적, 이름순) 인쇄\n");
//		printf("[7] 프로그램 종료\n");
//		printf("원하는 번호를 입력하시오(1~7) : ");
//		scanf("%d", &cmd);
//		switch (cmd) {
//		case 1:
//			DataAdd();
...
//	int end = 0;
//
//	//실제 요소 수 파악, 버퍼를 봐야하므로 만든 count함수 대신 직접 세기
...
//	for (i = 0; i < MAX_ST; i++)
//	{
//		//Head->next == NULL 조건을 가장 먼저 검사해야 함. 실제로 NULL인경우 조건문 에러
...
//		head = head->next;
//		buf[i] = *head;
//		buf[i].next = &buf[i + 1]; //중요 point
//	}
//	buf[i - 1].next = NULL; //마지막 요소 처리
//
//	return i;
//}
//
...
//	for (i = 0; i < n; i++)
//	{
//		printf("Queue[%d] = %d\n", rd, Queue[rd]);// ++을 쓰는 변수가 같은 line에 있으면 오류가능성
//		rd++;
//		rd %= MAX_Q;
//	}
//
...
//	else
//	{
//		Wrptr->next = (QUEUE*)malloc(sizeof(QUEUE)); //다음 주소에 공간 할당
//		if (Wrptr->next == NULL) return -1; //할당 실패
//		Wrptr = Wrptr->next; // 다음 공간으로 포인터 주소 이동
//	}
//
//	*Wrptr = *data; //새로 이동한 주소에 값 담기
//	Wrptr->next = NULL; // 마지막에 넣으므로 항상 next = 0
//	Q_cnt++;
//	return 1;
//}
//
...
//	for (i = 0; i < MAX_ST; i++) {
//		if (Hash_table[idx].id == -1) {//빈방 찾기 성공
//			Hash_table[idx] = *d;
//			return idx;
//		}
//		idx = idx + STEP;
...
//	for (i = 0; i < MAX_ST; i++) {
//		if (Hash_table[idx].id == id) return &Hash_table[idx];//찾기 성공
//		if (Hash_table[idx].id == -1) break;//찾기 실패
//		idx = idx + STEP;
//		if (idx >= MAX_ST) idx = 0;
//	}
//	return NULL;//해당 사번 없음
//}
//
//#endif
//
...
//#define STEP		1  // 테이블의 요소수와 서로 소 관계인 수
//#endif
//
//SCORE Hash_table[MAX_ST];
//
